...
plot1_function <- function(session, defaultPointSize, defaultLabelSize,defaultPalette,...){
  plotWidgetUI(session$ns("plot1"),
               ...
               settingsUI = softui::fluid_row(tags$p( "Grafiek instellingen" ),
                                              ...
                                              selectInput("palette_1", "Palette",choices=defined_palettes, selected = defaultPalette())
               )
               
  )
}

plot2_function <- function(session, defaultPointSize, defaultLabelSize,defaultPalette,...){
  plotWidgetUI(session$ns("plot2"),
               settingsUI = softui::fluid_row(tags$p( "Grafiek instellingen" ),
                                              ...
                                              numericInput("num_pointsize2", "Point size", value = defaultPointSize()),
                                              selectInput("palette_2", "Palette", choices=defined_palettes,selected = defaultPalette())
               )
               
  )
}

plot3_function <- function(session, defaultPointSize, defaultLabelSize,defaultPalette,...){
  plotWidgetUI(session$ns("plot3"),   
               settingsUI =  softui::fluid_row(tags$p( "Grafiek instellingen" ),
                                               ...
                                               selectInput("palette_3", "Palette", choices=defined_palettes,
...
callPlots <- function(input, output, session, raw_data){
  ...
  callModule(plotWidgetModule, "plot1",
             ...
             settings = reactive(
               list(
                 ...
                 yvar = "population",
                 reverse_order = FALSE,
                 palette_function = input$palette_1,
                 colors = NULL,
  ...
  callModule(plotWidgetModule, "plot2",
             ...
             settings = reactive(
               list(
                 ...
                 palette_function = input$palette_2,
                 colors = "#33B7A0",
                 base_size = input$num_basesize2,
                 label_size = 2.5,
                 point_size = input$num_pointsize2,
                 line_width = 1.2,
                 label_bars = TRUE,
                 label_k = FALSE,
                 ylab = "Populatie",
  ...
  callModule(plotWidgetModule, "plot3",
             ...
             settings = reactive(
               list(
                 ...
                 palette_function = input$palette_3,
                 colors = NULL,
...
defined_palettes <- list("Oceaan"="ocean.phase", "Viridis"="viridis", "Parula"="parula", "ocean.haline"="ocean.haline")
...
ui <- softui::simple_page(
  ...
  softui::fluid_row(
    column(12,
    softui::box(title="Dashboard instellen",closed=TRUE,
                 
                uiOutput("defaultSizes"),
                bucket_list(
                  header = "Grafieken weergeven",
                  group_name = "bucket_list_group",
                  orientation = "horizontal",
                  add_rank_list(
                    text = "Niet weergegeven",
                    labels = list("plot1_function", "plot2_function", "plot3_function"),
                    input_id =  "rank_list_onbekend" 
                  ),
                  add_rank_list(
                    text = "Links",
                    labels = NULL,
                    input_id =  "rank_list_left" 
                  ),
                  add_rank_list(
                    text = "Midden",
                    labels = NULL,
                    input_id =  "rank_list_middle" 
                  ),
                  add_rank_list(
                    text = "Rechts",
                    labels = NULL,
                    input_id =  "rank_list_right" 
                  )
                ),
                action_button("updateButton", label= "Update", status='success')
                )
    )
  ),
  softui::fluid_row(
...
server <- function(input, output, session) {
  ...
  defaultPalette <- reactiveVal("viridis")
  left_ranks<- reactiveVal( ) 
  middle_ranks<- reactiveVal( )  
  right_ranks<- reactiveVal( ) 
  ...
  output$defaultSizes <- renderUI({ 
    softui::fluid_row(
      ...
      selectInput(session$ns("defaultColors"), label="Palette", choices=defined_palettes, selected=defaultPalette())
  ...
  observeEvent(input$updateButton,ignoreInit = FALSE, ignoreNULL = FALSE,{
    ...
    defaultPalette(input$defaultColors)
    left_ranks(input$rank_list_left)
    middle_ranks(input$rank_list_middle)
    right_ranks(input$rank_list_right)
  }) 
  ...
  output$viz <- renderUI({
   softui::fluid_row( 
      column(4, 
             lapply(left_ranks(), function(x){ 
               y <- base::get(x) 
               y(session,defaultPointSize,defaultLabelSize,defaultPalette)
             })
              
      ),
      column(4,
             lapply(middle_ranks(), function(x){ 
               y <- base::get(x) 
               y(session,defaultPointSize,defaultLabelSize,defaultPalette)
             })
      ),
      column(4, 
             lapply(right_ranks(), function(x){ 
               y <- base::get(x) 
               y(session,defaultPointSize,defaultLabelSize,defaultPalette)
             })   
      )
  ...
  raw_data <- reactive({
   ...
   callPlots(input, output, session, raw_data)
